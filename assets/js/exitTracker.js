const setupExitTracking = () => {
  ...
  // Detect form submissions so we skip the exit beacon.
  // Firing a beacon during a form POST can race with the session save
  // and overwrite completedSteps in Redis (see PR #194 for history).
  ...
  function logPageExit(destination) {
    ...
    if (hasLoggedPageExit || isFormSubmitting) {
      return;
    }
