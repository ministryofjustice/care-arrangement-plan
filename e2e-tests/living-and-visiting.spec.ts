...
test.describe('Mostly Lives Page', () => {
  ...
  test.describe('Child mostly lives with Adult 1 and visits/stays with Adult 2', () => {
    test('Fill fields until section is complete', async ({ page }) => {
      ...
      await fillLivingAndVisitingSectionAndContinue(page, /will-overnights-happen/, 'Will the children stay overnight with ?', ['Yes'])
      ...
      await fillLivingAndVisitingSectionAndContinue(page, /will-daytime-visits-happen/, 'Will the children do daytime visits with ?', ['Yes'])
  ...
  test.describe('Child mostly lives with Adult 1 and does not visit or stay with Adult 2', () => {
    test('Fill fields until section is complete', async ({ page }) => {
      await fillLivingAndVisitingSectionAndContinue(page, /where-will-the-children-mostly-live/, 'Where will the children spend most of their time?', ['With Parent'])
      await fillLivingAndVisitingSectionAndContinue(page, /will-overnights-happen/, 'Will the children stay overnight with ?', ['No'])
      await fillLivingAndVisitingSectionAndContinue(page, /will-daytime-visits-happen/, 'Will the children do daytime visits with ?', ['No'])
      await expect(page).toHaveURL(/\/make-a-plan/);
      await expect(page.locator('#livingAndVisiting-taskList-1-status')).toHaveText(/Completed/i)
