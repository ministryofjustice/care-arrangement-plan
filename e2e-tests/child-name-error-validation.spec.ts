...
test.describe('About the children Page Validation', () => {
    test('An error should be shown above an empty input', async ({ page }) => {
      ...
      await expect(page.locator(`#${ERROR_ID.child1}`)).toContainText("Enter first name for the first child");
    });
...
test.describe('Multiple Children Input Validation', () => {
  test('should show error that matches child input', async ({ page }) => {
    ...
    await expect(page.locator(`#${ERROR_ID.child3}`)).toContainText("Enter first name for the third child");
    await expect(page.locator(`#${ERROR_ID.child4}`)).toContainText("Enter first name for the fourth child");
    await expect(page.locator(`#${ERROR_ID.child5}`)).toContainText("Enter first name for the fifth child");
    await expect(page.locator(`#${ERROR_ID.child6}`)).toContainText("Enter first name for the sixth child");
  });
});
