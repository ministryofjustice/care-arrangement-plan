...
test.describe('Task List', () => {
  ...
  test('should set the browser page title for the task list', async ({ page }) => {
    await expect(page).toHaveURL(/\/do-whats-best/);

    // Continue through the flow until the task list is reached
    await page.getByRole('checkbox', { name: /I will put my children.s needs first/i }).check();
    await page.getByRole('button', { name: /continue/i }).click();
    await page.getByLabel(/no/i).first().check();
    await page.getByRole('button', { name: /continue/i }).click();
    ...
    await expect(page).toHaveURL(/\/make-a-plan/);
    await expect(page).toHaveTitle(/Child arrangement plan with child\(ren\) details/);
  });
