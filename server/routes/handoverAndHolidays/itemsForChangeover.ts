...
import { ItemsForChangeoverAnswer } from '../../@types/session';
...
const getFieldName = (childIndex: number) => `${formFields.ITEMS_FOR_CHANGEOVER}-${childIndex}`;
...
const itemsForChangeoverRoutes = (router: Router) => {
  router.get(paths.HANDOVER_HOLIDAYS_ITEMS_FOR_CHANGEOVER, checkFormProgressFromConfig(FORM_STEPS.HANDOVER_HOLIDAYS_ITEMS_FOR_CHANGEOVER), (request, response) => {
    ...
    const existingAnswers = handoverAndHolidays?.itemsForChangeover;
  ...
  router.post(
    ...
    (request, response) => {
      ...
      const byChild: Record<number, ItemsForChangeoverAnswer> = {};
