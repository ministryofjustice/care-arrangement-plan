  ...
  describe('GET', () => {
    ...
    it('should render the will overnights happen page for the primary adult', async () => {
      ...
      expect(dom.window.document.querySelector('h1')).toHaveTextContent(
        `Will the children stay overnight with ${session.initialAdultName}?`,
      );
    ...
    it('should render the will overnights happen page for the initial adult', () => {
      ...
      return request(app)
        ...
        .expect('Content-Type', /html/)
        .expect((response) => {
          expect(response.text).toContain(`Will the children stay overnight with ${session.secondaryAdultName}?`);
        });
    });
